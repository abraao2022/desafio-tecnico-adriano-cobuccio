...
const validationSchema = Yup.object({
    to_customer_id: Yup.number().required('ID do cliente de destino é obrigatório'),
...
export default function DepositModal({ open, handleClose }: { open: boolean; handleClose: () => void }) {
                ...
                <Box sx={style}>
                    <Formik
                        ...
                        onSubmit={async (values, { setSubmitting, setStatus }) => {
                            try {
                                await transactionsService.deposit(values.to_customer_id, values.amount, values.description).then(() => {
                                    handleClose();
                                });
                            } catch (error: unknown) {
